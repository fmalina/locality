...
THIRD_POS_CHARS = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'S', 'T',
                   'U', 'W']
...
def hasdigit(val):
    # Other types of locations rarely contain digits.
    if [x for x in val if x.isdigit()]:
        return True
    return False


def normalize_postcode(val):
    return val.replace(' ', '').upper().strip()


def space_postcode(p):
    """Add space to postcode in conventional position"""
    cut = {5: 2, 6: 3, 7: 4}[len(p)]
    return p[:cut]+'+'+p[cut:]


def parse_postcode(postcode, strict=True, incode_mandatory='Incode mandatory'):
    ...
    Original from Simon Brunning <simon@brunningonline.net>, 14.4.'04
    """

    postcode = normalize_postcode(postcode)

    if len(postcode) > 7:
        raise ValueError(incode_mandatory)
